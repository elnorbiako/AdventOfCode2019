...
//        String b = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";
        ...
        //System.out.println(cableB[1]);

        Set<Point> pointsA = new LinkedHashSet<>();
        Set<Point> pointsB = new LinkedHashSet<>();

        Point startA = new Point(0,0);
        Point startB = new Point(0,0);

//        System.out.println(move("R75", start, pointsA));
        ...
        Set<Point> commonPoints = new LinkedHashSet<>();
...
//        System.out.println(pointsB);
    ...
    public static Point move (String code, Point lastPlace, Set<Point> pointSet) {

        char letter = code.charAt(0);
        int steps = Integer.parseInt(code.substring(1));

        if (letter == 'R') {
            for (int i = lastPlace.x; i < lastPlace.x + steps; i++ ) {
                pointSet.add(new Point(i, lastPlace.y));
            }
            int x = lastPlace.x + steps;
            return new Point(x,lastPlace.y);
        }

        if (letter == 'U') {
            for (int j = lastPlace.y; j < lastPlace.y + steps; j++ ) {
                pointSet.add(new Point(lastPlace.x, j));
            }
            int y = lastPlace.y + steps;
            return new Point(lastPlace.x,y);
        }

        if (letter == 'L') {
            for (int k = lastPlace.x; k > lastPlace.x - steps; k-- ) {
                pointSet.add(new Point(k, lastPlace.y));
            }
            int x = lastPlace.x - steps;
            return new Point(x,lastPlace.y);
        }
        ...
        if (letter == 'D') {
            for (int l = lastPlace.y; l > lastPlace.y - steps; l-- ) {
                pointSet.add(new Point(lastPlace.x, l));
            }
            int y = lastPlace.y - steps;
            return new Point(lastPlace.x,y);
        }
    ...
    public static Point move (String code, Point lastPlace, List<Point> pointList) {
        ...
        if (letter == 'R') {
            for (int i = lastPlace.x; i < lastPlace.x + steps; i++ ) {
                pointList.add(new Point(i, lastPlace.y));
            }
            int x = lastPlace.x + steps;
            return new Point(x,lastPlace.y);
        }
        ...
        if (letter == 'U') {
            for (int j = lastPlace.y; j < lastPlace.y + steps; j++ ) {
                pointList.add(new Point(lastPlace.x, j));
            }
            int y = lastPlace.y + steps;
            return new Point(lastPlace.x,y);
        }
        ...
        if (letter == 'L') {
            for (int k = lastPlace.x; k > lastPlace.x - steps; k-- ) {
                pointList.add(new Point(k, lastPlace.y));
            }
            int x = lastPlace.x - steps;
            return new Point(x,lastPlace.y);
        }
        ...
        if (letter == 'D') {
            for (int l = lastPlace.y; l > lastPlace.y - steps; l-- ) {
                pointList.add(new Point(lastPlace.x, l));
            }
            int y = lastPlace.y - steps;
            return new Point(lastPlace.x,y);
        }
